...
namespace PopupSystem
{
    public class AlertPopup : AlertPopupBase
    {
        ...
        [Header("TEXTS")] [SerializeField] private TextMeshProUGUI acceptButtonText;
        ...
        [Header("ERROR SPRITES")] [SerializeField]
        ...
        private async UniTask Close(Action callback = null)
        {
            SetInteractable(false);
            await CloseSelf(callback);
        }
        ...
        {
            Close(() => onAccept.OnNext(unit)).Forget();
        }

        private void HandleRetry(Unit unit)
        {
            Close(() => onRetry.OnNext(unit)).Forget();
        }

        private void HandleReject(Unit unit)
        {
            Close(() => onReject.OnNext(unit)).Forget();
        }
    }
}
